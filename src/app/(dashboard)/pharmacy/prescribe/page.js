...
import { Pill, Save, ArrowLeft, Search, ShoppingCart, Trash2, X, Plus, Minus } from 'lucide-react';
...
export default function PrescribeMedicinePage() {
    ...
    const addToCart = (med) => {
        ...
        if (existing) {
            ...
            setCart([...cart, { medicineId: med.id, name: med.name, unitPrice: med.mrp, qty: 1, maxStock: med.stock, drugCode: med.drugCode }]);
        }
        setDrugSearch('');
        setDrugResults([]);
    };
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    ...
                    <div className="card" style={{ padding: '24px' }}>
                        ...
                        <div style={{ position: 'relative', marginBottom: '20px' }}>
                            <input type="text" placeholder="Search drug inventory by name or generic..."
                                    ...
                                    {drugResults.map(med => (
                                        <div key={med.id} onClick={() => addToCart(med)}
                                            ...
                                            <div>
                                                ...
                                                <div style={{ fontSize: '12px', color: 'var(--color-text-secondary)' }}>{med.genericName || med.category} · Stock: {med.stock}</div>
                                            </div>
                                        ...
                                        {cart.map(item => (
                                            <tr key={item.medicineId} style={{ borderBottom: '1px solid var(--color-border-light)' }}>
                                                <td style={{ padding: '12px' }}>
                                                    ...
                                                    <div style={{ fontSize: '11px', color: 'var(--color-text-muted)' }}>{item.drugCode}</div>
                                                </td>
                                                <td style={{ padding: '12px' }}>
